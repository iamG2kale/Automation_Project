...
apache2Enable=`systemctl list-unit-files | grep enabled | grep apache2 | awk '{print $2}'`
if [[ ! $apache2Enable == "enabled" ]]
then
        printf "Enabling the Apache2 service in systemctl.\n"
        systemctl enable apache2.service
else
        printf "Apache2 Service is already enabled in systemctl.\n"
fi
...
tar cvfz $MYNAME-httpd-logs-$timestamp.tar  access.log > /dev/null 2>&1
